	...
	_createToolbarLinkButton() {
		...
		editor.ui.componentFactory.add( 'DocumentLink', locale => {
			...
			this.listenTo( button, 'execute', () => {
				...
				const {model, data, model:{document:{selection}}} = editor;
				const range = selection.getFirstRange();
				if (range.end.isEqual(range.start)) {
					const notifications = editor.plugins.get('Notification');
					notifications.showWarning('Please select text before creating a document link.');
				} else {
					// If we DO have a selection, defer to the onclick handler (pop up a UI, do whatever)
					if (this.toolbarButtonCallback){
